  ...
  return (
    ...
    <Popover>
      <PopoverTrigger>
        ...
        >
          <Image src="/tradelogo.png" alt="logo" width={40} height={40} />
          <h2
      ...
      <PopoverContent className="ml-7 p-4">
        ...
        <div>
            {teamList?.map((team,index)=>(
                <h2 key={index} className={`p-2 hover:bg-blue-500 hover:text-white rounded-lg mb-1 cursor-pointer ${activeTeam?._id==team._id&&'bg-blue-500 text-white'}`}
        ...
        <div>
            {menu.map((item,index)=>(
                <h2 key={index}className="flex gap-2 items-center p-2 hover:bg-gray-100 rounded-lg cursor-pointer text-sm"
